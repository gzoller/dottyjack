...
class TupleCollKeys() extends FunSuite:
  ...
  */
  ...
  test("Parameterized trait") {
    ...
    val inst = Map(t1 -> t2)
    // val inst = Map(AThing("A",3)->"foom")
    // val r = RType.of[Map[Thing[String,Int],String]].asInstanceOf[info.MapLikeInfo]
    // val inst = ParamWrapper(Map(t1 -> t2))
    val js = sj.render(inst)
    // println(js)
    // println(sj.read[Map[Thing[String,Int],String]](js))
    assertEquals(
    ...
    // assert(inst == sj.read[ParamWrapper[T1,T1]](js))
    assert(inst == sj.read[Map[(T1,T1),(T1,T1)]](js))
  }
  ...
  //   val inst = Map(AThing("a",APart(1.2)) -> "Foo")
  //   val js = sj.render(inst)
  //   println(js)
  // }
  test("Parameterized trait having parameterized trait members") {
    ...
    val t2: (T2, T2) =
      (AThing("yep", APart(4.5)), AThing("yikes", APart(6.7)))
    println("HERE: "+RType.of[Map[(T2,T2),(T2,T2)]])
    val inst = Map(t1 -> t2)
    // val inst = ParamWrapper(Map(t1 -> t2))
    val js = sj.render(inst)
    println(js)
