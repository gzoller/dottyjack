...
class Custom extends FunSuite:
  ...
  test("allowPermissivePrimitives()") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Permissive primitives not supported for Json4s"){
  ...
  test("withDefaultHint()") {
    val sj2 = co.blocke.dottyjack.DottyJack(Json4sFlavor()).withDefaultHint("kind")
    val dbo = new JObject(
      List(
  ...
  test("withHints()") {
    val sj2 = co.blocke.dottyjack.DottyJack(Json4sFlavor()).withHints(
