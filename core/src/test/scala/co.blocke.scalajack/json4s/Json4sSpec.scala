...
import co.blocke.dottyjack.json4s._
import co.blocke.scala_reflection.RType

class Json4sSpec extends FunSuite:

  val sj = co.blocke.dottyjack.DottyJack(Json4sFlavor())
  ...
  test("Null Maps work") {
    ...
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Map keys cannot be null."){
      sj.render(inst2)
    }
  }
  ...
  test("Bad JValueBuilder access") {
    val b = JValueBuilder()
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("No value set for internal json4s builder"){
      b.result()
    }
  }
  ...
  test("Malformed error works") {
    ...
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Cannot parse an Int from value"){
  ...
  test("Unexpected error works") {
    val js4s = JArray(List(JInt(3), JDouble(3.1)))
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected object here, not 'JArray(List(JInt(3), JDouble(3.1)))'"){
      sj.read[Player](js4s)
    }
  }
  ...
  test("Broken hint mod (no class)") {
    ...
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Couldn't marshal class for BogusAddress"){
  ...
  test("No type hint in trait") {
    ...
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Type hint '_hint' not found"){
  ...
  test("Non-scalars can't be map keys for Json4s") {
    val p = Player("Fred", 1)
    val m = Map(p -> 3)
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Json4s type org.json4s.JsonAST$JObject is not supported as a Map key"){
      sj.render(m)
    }
    val js4s = JObject(
      List(
    ...
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Only scalar values are supported as BSON Map keys"){
  ...
  test("Externalized type hints work (with type modifier!)") {
    import model._
    val scalaJack = co.blocke.dottyjack.DottyJack(Json4sFlavor()).withTypeValueModifier(
