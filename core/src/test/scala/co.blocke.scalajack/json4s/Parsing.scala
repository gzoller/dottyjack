...
import org.json4s._
import co.blocke.dottyjack.json4s.Json4sFlavor
...
class Parsing() extends FunSuite:
  ...
  test("Non-String value where String expected") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected string here, not 'JInt(5)'"){
  ...
  test("Non-List value where List expected") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected list here, not 'JInt(5)'"){
  ...
  test("Non-tuple value where tuple expected") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected tuple (list) here, not 'JInt(5)'"){
  ...
  test("Non-Map value where Map expected") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected map here, not 'JInt(5)'"){
  ...
  test("Non-Boolean value where Boolean expected") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected boolean here, not 'JInt(5)'"){
      sj.read[Boolean](JInt(5))
    }
  }

  test("Non-Number value where Number expected") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected number here, not 'JString(x)'"){
      sj.read[Int](new JString("x"))
    }
  }

  test("Attempt to scan for type hint on a non-object") {
    interceptMessage[co.blocke.dottyjack.ScalaJackError]("Expected object here, not 'JInt(5)'"){
